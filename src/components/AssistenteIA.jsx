...
function AssistenteIA({ onClose }) {
  const [mensagens, setMensagens] = useState([{ tipo: 'ia', texto: 'Olá! 💅 Sou sua assistente virtual. Como posso te ajudar hoje?' }]);
  ...
  const enviarMensagem = async (texto) => {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: `Contexto: ${JSON.stringify(agendamentos)}. Pergunta: ${msg}` }] }]
        })
      });
      ...
      setMensagens(p => [...p, { tipo: 'ia', texto: resposta || 'Não entendi.' }]);
    } catch (err) {
      // Usando console.error(err) garante que a variável 'err' foi utilizada, eliminando o aviso
      console.error('Erro na requisição:', err);
      setMensagens(p => [...p, { tipo: 'ia', texto: 'Erro de conexão. Verifique a chave VITE_GEMINI_API_KEY no painel da Vercel.' }]);
    } finally {
      setCarregando(false);
    }
  };
  ...
  return (
    <div className="assistente-wrapper">
      <div className="assistente-header">
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#7c3aed', fontSize: '20px' }}>✕</button>
      </div>

      <div className="assistente-chat">
        <div className="chat-mensagens">
          {mensagens.map((m, i) => (
            <div key={i} className={`mensagem ${m.tipo}`}><div className="mensagem-balao">{m.texto}</div></div>
          ))}
        ...
        <div className="chat-input-area">
          <input className="chat-input" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && enviarMensagem()} placeholder="Pergunte algo..." />
          <button className="chat-send-btn" onClick={() => enviarMensagem()}>➤</button>
        </div>
      </div>
    </div>
  );
